...
def create_remote_folder(remote_folder, ip, username, ssh_key="id_rsa"):
    """Ensure that the remote folder exists."""
    try:
...
def transfer_to_server(local_path, remote_path, ip, username, ssh_key="id_rsa"):
    try:
        ...
        scp_command = f"scp -i C:\\Users\\onwar\\.ssh\\id_rsa {local_path} {username}@{ip}:{remote_path}"
...
def process_study_folder(root_folder, study_folder, server_ip, username, remote_folder, ssh_key="id_rsa"):
...
def main():
    ...
    server_ip = "35.154.2.160"
    username = "ubuntu"
    remote_folder = "/opt/minio/pathflowdx-dev/input/"
    ssh_key = "id_rsa"
